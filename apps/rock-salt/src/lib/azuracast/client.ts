...
export class AzuraCastClient {
  ...
  /**
   * Add media to a named playlist (case-insensitive). Falls back to default.
   */
  async addToPlaylistByName(mediaId: number, playlistName: string): Promise<void> {
    const playlists = await this.getPlaylists()
    const normalizedName = playlistName.trim().toLowerCase()
    const matched = playlists.find((p: any) =>
      String(p.name || '').trim().toLowerCase() === normalizedName
    )
    const targetPlaylistId = matched?.id
    await this.addToPlaylist(mediaId, targetPlaylistId)
  }
