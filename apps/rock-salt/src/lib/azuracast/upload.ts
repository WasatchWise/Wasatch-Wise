...
export function createMetadataFromSubmission(
  ...
  const primaryGenre = submission.genre_preferences?.[0] || 'Unsorted'
  const album = `The Rock Salt — ${primaryGenre}`
...
export async function processSubmissionToPlaylist(
  ...
  // Add to genre playlist if it exists, otherwise default
  const client = apiKey
    ? new AzuraCastClient({ apiKey })
    : new AzuraCastClient()
  ...
  try {
    const primaryGenre = submission.genre_preferences?.[0] || 'Unsorted'
    await client.addToPlaylistByName(mediaId, primaryGenre)
