...
function isValidUrl(url: string | null | undefined): boolean {
  if (!url) return false
  // Check if it's a valid URL (starts with http/https or is a valid domain)
  try {
    ...
    return (
      ...
      >
        ...
        <div className="p-6">
          ...
          {band.bio && (
            <p
              className="text-white/80 text-sm mb-4 line-clamp-2 leading-relaxed"
              title={band.bio}
            >
              {band.bio}
            </p>
          ...
          {band.band_links?.length ? (
            <div className="flex flex-wrap gap-2">
              {band.band_links.filter(link => isValidUrl(link.url)).slice(0, 3).map((link) => (
                <a
                  key={link.id}
    ...
    return (
      ...
      >
        ...
        <div className="p-5">
          ...
          {band.band_links?.length ? (
            <div className="flex flex-wrap gap-2">
              {band.band_links.filter(link => isValidUrl(link.url)).slice(0, 2).map((link) => (
                <a
                  key={link.id}
                  href={normalizeUrl(link.url)}
                  target="_blank"
