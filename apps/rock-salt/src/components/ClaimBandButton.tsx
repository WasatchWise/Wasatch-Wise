...
export default function ClaimBandButton({ bandId, bandName, isClaimed }: ClaimBandButtonProps) {
  ...
  const handleClaim = async () => {
    ...
    if (result.success) {
      setSuccess(true)
      setTimeout(() => {
        router.push('/dashboard?claimed=band')
      }, 2000)
    } else {
      ...
      {!user && (
        <div className="mt-4 pt-4 border-t border-indigo-100 dark:border-indigo-900/50">
          <p className="text-sm text-gray-500 dark:text-gray-400">
            ...
            <a
              href={`mailto:music@therocksalt.com?subject=Claim Request: ${bandName}&body=I&apos;m having trouble claiming the page for ${bandName}. My email is: `}
              className="text-indigo-600 dark:text-indigo-400 hover:underline font-medium"
            >
              Request a manual claim review →
            </a>
          </p>
        </div>
      )}
