...
export default function MusicSubmissionForm() {
  ...
  const validateForm = (formData: FormData): FormErrors => {
    ...
    if (!bio || bio.length < 50 || bio.length > 300) {
      errors.bio = 'Sound descriptor must be between 50 and 300 characters'
    }

    const description = formData.get('description') as string
    if (!description || description.length < 80 || description.length > 1500) {
      errors.description = 'Archive description must be between 80 and 1500 characters'
    }
    ...
    if (!musicFile || musicFile.size === 0) {
      errors.musicFile = 'Master file is required'
    } else {
      if (musicFile.size > 26214400) { // 25MB
        errors.musicFile = 'Master file must be less than 25MB'
      }
      if (!['audio/wav', 'audio/x-wav', 'audio/flac', 'audio/x-flac', 'audio/mpeg', 'audio/mp3'].includes(musicFile.type)) {
        errors.musicFile = 'Master file must be WAV, FLAC, or MP3'
      }
    }
      ...
      <section className="space-y-6">
        ...
        <div>
          <label htmlFor="bio" className="block text-sm font-medium mb-1 text-gray-900">
            Sound Descriptor <span className="text-red-500">*</span>
          </label>
          <textarea
            id="bio"
            name="bio"
            ...
            maxLength={300}
            className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent text-gray-900 placeholder-gray-400"
            placeholder="Sound descriptor. 50 characters minimum."
            onChange={(e) => setBioLength(e.target.value.length)}
          />
          <div className="flex justify-between text-sm mt-1">
            <p className="text-gray-500">
              Sound descriptor. 50 characters minimum. Example: "Post-hardcore. Mathcore adjacent. Instrumental."
            </p>
        ...
        <div>
          <label htmlFor="description" className="block text-sm font-medium mb-1 text-gray-900">
            Archive Description <span className="text-red-500">*</span>
          </label>
          <textarea
            id="description"
            name="description"
            ...
            maxLength={1500}
            className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent text-gray-900 placeholder-gray-400"
            placeholder="Genre classification. Primary influences. Years active. No promotional language."
            onChange={(e) => setDescriptionLength(e.target.value.length)}
          />
          <div className="flex justify-between text-sm mt-1">
            <p className="text-gray-500">Genre classification. Primary influences. Years active. No promotional language.</p>
            <p className={`${descriptionLength < 80 ? 'text-red-500' : 'text-gray-500'}`}>{descriptionLength}/1500 (min 80)</p>
          </div>
        ...
        <div>
          ...
          {photoPreview && (
            <div className="mt-2">
              <img src={photoPreview} alt="Preview of uploaded band photo" className="max-w-xs rounded-lg shadow" />
            </div>
          )}
        </div>
      </section>
      ...
      <section className="space-y-6">
        ...
        <div>
          <label htmlFor="musicFile" className="block text-sm font-medium mb-1 text-gray-900">
            Music Upload <span className="text-red-500">*</span>
          </label>
          <input
            type="file"
            id="musicFile"
            name="musicFile"
            required
            accept="audio/wav,audio/x-wav,audio/flac,audio/x-flac,audio/mpeg,audio/mp3"
            className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent text-gray-900 placeholder-gray-400"
            onChange={handleMusicChange}
          />
          {errors.musicFile && <p className="text-red-500 text-sm mt-1">{errors.musicFile}</p>}
          <p className="text-gray-500 text-sm mt-1">WAV, FLAC, or MP3 (max 25MB).</p>
          {musicFileName && (
        ...
        <div>
          ...
          <textarea
            ...
            rows={3}
            className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent text-gray-900 placeholder-gray-400"
            placeholder="Track context. Recording date. Personnel."
          />
        </div>
      ...
      <section className="space-y-6">
        ...
        <div>
          <label className="flex items-start space-x-2">
            ...
            />
            <span className="text-sm text-gray-900">
              Send me operational updates and notices from RockSalt
            </span>
          </label>
        </div>
      </section>
