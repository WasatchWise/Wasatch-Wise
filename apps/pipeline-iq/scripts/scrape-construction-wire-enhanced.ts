...
type Vertical = 'hotel' | 'multifamily' | 'senior_living' | 'student_housing'
const VERTICAL_ARG = process.argv.find(arg => arg.startsWith('--vertical='))?.split('=')[1]?.trim()
const VERTICAL: Vertical = ((
  VERTICAL_ARG ||
  process.env.CW_VERTICAL ||
  'hotel'
) as string) as Vertical
const ALLOWED_VERTICALS: Vertical[] = ['hotel', 'multifamily', 'senior_living', 'student_housing']
if (!ALLOWED_VERTICALS.includes(VERTICAL)) {
  throw new Error(`Invalid --vertical="${VERTICAL}". Allowed: ${ALLOWED_VERTICALS.join(', ')}`)
}
...
class EnhancedConstructionWireScraper {
  ...
  getStats() {
    return {
      projectsScraped: this.projectsScraped,
      projectsInserted: this.projectsSaved,
      projectsUpdated: this.projectsUpdated,
      errors: this.errors,
    }
  }
  ...
  private getVerticalReportCandidates(vertical: Vertical): string[] {
    // CW URLs can vary by account/feature set. We try a few likely candidates per vertical.
    // The goal is: land on a project listing page where `a.title` exists after clicking "Search Projects".
    const base = 'https://www.constructionwire.com'
    const candidatesByVertical: Record<Vertical, string[]> = {
      hotel: [
        `${base}/Client/Report/Hotels`,
        `${base}/Client/Report/Hotel`,
        `${base}/Client/Report/Hospitality`,
        base,
      ],
      multifamily: [
        `${base}/Client/Report/Multifamily`,
        `${base}/Client/Report/MultiFamily`,
        `${base}/Client/Report/Apartments`,
        `${base}/Client/Report/Residential`,
        base,
      ],
      senior_living: [
        `${base}/Client/Report/SeniorLiving`,
        `${base}/Client/Report/Senior_Living`,
        `${base}/Client/Report/AssistedLiving`,
        `${base}/Client/Report/Healthcare`,
        base,
      ],
      student_housing: [
        `${base}/Client/Report/StudentHousing`,
        `${base}/Client/Report/Student_Housing`,
        `${base}/Client/Report/UniversityHousing`,
        `${base}/Client/Report/Education`,
        base,
      ],
    }

    return candidatesByVertical[vertical]
  }

  private async isOnListingPage(): Promise<boolean> {
    if (!this.page) return false
    try {
      await this.page.waitForSelector('a.title', { timeout: 5000 })
      return true
    } catch {
      try {
  ...
  async navigateToVertical(vertical: Vertical) {
    if (!this.page) throw new Error('Browser not initialized')

    console.log(`\n🧭 Navigating to ${vertical} projects...`)
    ...
    try {
      ...
      const candidates = this.getVerticalReportCandidates(vertical)
      ...
      for (const url of candidates) {
        try {
          ...
          // Many CW reports require clicking "Search Projects" to populate the listing.
          ...
          if (await this.isOnListingPage()) {
            console.log(`✅ On ${vertical} projects page`)
            await this.screenshot(`${vertical}-projects-page`)
            return
          }
        } catch (e) {
          // Try next candidate
      ...
      await this.screenshot(`${vertical}-navigation-failed`)
      throw new Error(`Could not navigate to a ${vertical} listing page (tried ${candidates.length} candidates).`)
    } catch (error: any) {
  ...
  async scrapeProjects(maxProjects = 100, fetchDetails = false): Promise<ScrapedProject[]> {
    ...
    console.log(`\n📊 Scraping ${VERTICAL} projects (max: ${maxProjects})...`)
          ...
          try {
            ...
            if (project) {
              ...
              if (fetchDetails && raw.href) {
                ...
                // Merge detected services/amenities from detail page
                if (details.servicesNeeded && details.servicesNeeded.length > 0) {
                  project.services_needed = details.servicesNeeded
                }
  ...
  parseProjectData(raw: any): ScrapedProject | null {
    try {
      ...
      let projectTypes: string[] = [VERTICAL]
  ...
  async scrapeProjectDetails(projectUrl: string): Promise<{
    ...
    try {
      ...
      result.servicesNeeded = pageData.servicesNeeded || []
      ...
      if (result.servicesNeeded && result.servicesNeeded.length > 0) {
        extractedInfo.push(`Services: ${result.servicesNeeded.join(', ')}`)
      }
  ...
  async saveToDatabase(projects: ScrapedProject[]) {
    ...
    for (const project of projects) {
      try {
        ...
        // Check if project exists (dedupe by cw_project_id)
        const { data: existing } = await supabase
          .from('projects')
          .select('id, project_type, raw_data')
        ...
        if (existing) {
          ...
          const existingTypes: string[] = Array.isArray(existing.project_type) ? existing.project_type : []
          const incomingTypes: string[] = Array.isArray(projectData.project_type) ? (projectData.project_type as any) : []
          const mergedTypes = Array.from(new Set([...existingTypes, ...incomingTypes].filter(Boolean)))

          const existingRaw = (existing.raw_data || {}) as any
          const existingVerticals = Array.isArray(existingRaw?.source_verticals) ? existingRaw.source_verticals : []
          const mergedVerticals = Array.from(new Set([...existingVerticals, VERTICAL].filter(Boolean)))

          const existingServices = Array.isArray(existingRaw?.services_detected) ? existingRaw.services_detected : []
          const incomingServices = Array.isArray(project.services_needed) ? project.services_needed : []
          const mergedServices = Array.from(new Set([...existingServices, ...incomingServices].filter(Boolean)))

          const mergedRawData = {
            ...existingRaw,
            // Always keep the last seen vertical and the full set of verticals
            source_vertical: VERTICAL,
            source_verticals: mergedVerticals,
            services_detected: mergedServices,
            classification: classification,
            // Keep enrichment/other historical keys in existingRaw; update original/contact snapshot
            original: {
              ...(existingRaw?.original || {}),
              ...((project.raw_data?.original || {}) as any),
  ...
  async clickNextPage(): Promise<boolean> {
    ...
    try {
      // Capture a "page fingerprint" so we can detect when pagination does NOT advance.
      const prevUrl = this.page.url()
      const prevFirstHref = await this.page
        .$eval('a.title', el => (el as HTMLAnchorElement).getAttribute('href') || '')
        .catch(() => '')

      const prevActivePageText = await this.page
        .evaluate(() => {
          const activeEl =
      ...
      const clickResult = await this.page.evaluate(() => {
        const isDisabled = (el: Element) => {
          ...
          const className = (he.className || '').toString()
          const ariaDisabled = (he.getAttribute('aria-disabled') || '').toLowerCase()
          return (
            className.includes('disabled') ||
            he.getAttribute('disabled') !== null ||
            ariaDisabled === 'true'
          )
        }
        ...
        if (candidates.length === 0) return { clicked: false, reason: 'no_next_candidate' as const }

        // Prefer anchors / buttons (clickable), otherwise click the first match.
        const preferred = candidates.find(el => el.tagName === 'A' || el.tagName === 'BUTTON') || candidates[0]
        ;(preferred as HTMLElement).click()
        return { clicked: true, reason: 'clicked' as const }
      })
      ...
      // CW often uses in-page JS navigation; wait for either URL, active page, or first result href to change.
      await Promise.race([
        this.page.waitForNavigation({ waitUntil: 'networkidle2', timeout: 15000 }).catch(() => undefined),
        this.page.waitForFunction(
          ({ prevUrl, prevFirstHref, prevActivePageText }) => {
            const urlChanged = window.location.href !== prevUrl
            const firstHref = (document.querySelector('a.title') as HTMLAnchorElement | null)?.getAttribute('href') || ''
            const firstChanged = !!prevFirstHref && firstHref !== prevFirstHref
            ...
            const activeChanged = !!prevActivePageText && activeText !== prevActivePageText

            return urlChanged || firstChanged || activeChanged
          },
          { timeout: 15000 },
          { prevUrl, prevFirstHref, prevActivePageText }
        ).catch(() => undefined),
      ...
      const newFirstHref = await this.page
        .$eval('a.title', el => (el as HTMLAnchorElement).getAttribute('href') || '')
        .catch(() => '')

      // If we still see the exact same first result, pagination likely didn't advance.
      if (prevFirstHref && newFirstHref && prevFirstHref === newFirstHref) {
        console.log(`   ⚠️  Next-page click did not change results (same first href: ${newFirstHref}). Stopping pagination to avoid duplicates.`)
        await this.screenshot('pagination-stuck')
        return false
      }
...
async function main() {
  ...
  try {
    // Record a scrape log for visibility (best-effort)
    try {
      const { data: log, error } = await (supabase as any)
        .from('scrape_logs')
        .insert({
          organization_id: ORGANIZATION_ID,
          source: 'construction_wire_enhanced',
          status: 'running',
          metadata: {
            vertical: VERTICAL,
            strategy: DEEP_TRAWL ? 'deep' : 'standard',
        ...
        .select('id')
        .single()
      if (!error && log?.id) scrapeLogId = log.id
    } catch {
      // ignore
    }
    ...
    await scraper.init()
    await scraper.login()
    await scraper.navigateToVertical(VERTICAL)
    ...
    // Update scrape log (best-effort)
    try {
      if (scrapeLogId) {
        const durationSeconds = (Date.now() - scrapeStartedAtMs) / 1000
        ...
        await (supabase as any)
          ...
          .update({
            ...
            projects_inserted: stats.projectsInserted || 0,
            projects_updated: stats.projectsUpdated || 0,
    ...
    // Update scrape log failure (best-effort)
    try {
      if (scrapeLogId) {
        const durationSeconds = (Date.now() - scrapeStartedAtMs) / 1000
        await (supabase as any)
