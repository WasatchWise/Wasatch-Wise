-- ============================================
-- WASATCHWISE + DAROS COMPLETE MIGRATION
...
-- Update email_captures to match WasatchWise structure (safe migration)
DO $$ 
BEGIN
  -- Add missing columns if they don't exist
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns 
                 WHERE table_schema = 'public' AND table_name = 'email_captures' AND column_name = 'name') THEN
    ALTER TABLE email_captures ADD COLUMN name TEXT;
  END IF;
  
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns 
                 WHERE table_schema = 'public' AND table_name = 'email_captures' AND column_name = 'organization') THEN
    ALTER TABLE email_captures ADD COLUMN organization TEXT;
  END IF;
  
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns 
                 WHERE table_schema = 'public' AND table_name = 'email_captures' AND column_name = 'role') THEN
    ALTER TABLE email_captures ADD COLUMN role TEXT;
  END IF;
  
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns 
                 WHERE table_schema = 'public' AND table_name = 'email_captures' AND column_name = 'lead_magnet') THEN
    ALTER TABLE email_captures ADD COLUMN lead_magnet TEXT;
  END IF;
  
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns 
                 WHERE table_schema = 'public' AND table_name = 'email_captures' AND column_name = 'converted_to_client') THEN
...
-- AND table_name IN ('clients', 'projects', 'districts', 'artifacts', 'controls', 'stakeholder_matrix')
-- ORDER BY table_name;
