...
## 🤝 Two-Agent Workflow (You + Cursor)

**You run in the browser.** Cursor runs in the IDE and owns the codebase. You are co-workers.

- **You:** Execute tests in Chrome, click through flows, run Lighthouse/a11y, **correct** things in the browser where possible. Report back with pass/fail, screenshots, and concrete issues.
- **Cursor:** Fixes code, Supabase, env, APIs; assigns you tasks; interprets your reports and ships fixes.

**Report back to Cursor** using the format in **`CURSOR_CHROME_AGENT_COORDINATION.md`** (section "Chrome Extension → Cursor"). Put anything that needs a code/API/data change in **NEEDS CURSOR** so Cursor can fix it.
