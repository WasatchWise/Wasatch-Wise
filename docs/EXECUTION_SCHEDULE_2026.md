# Wasatch Wise — Execution Schedule 2026
## Stop Thinking. Start Doing.

**Started:** Tuesday, February 10, 2026
**Last updated:** Wednesday, February 11, 2026
**Target:** $50K MRR, 10K email subscribers, national K-12 AI governance authority by Feb 2027

---

## WEEK 1: STOP THE BLEEDING
**Goal:** All 7 P0 items DONE by Friday 5pm. No exceptions.

### TUESDAY FEB 10
| Time | Task | Done? |
|------|------|-------|
| 2-3 hrs | **Install GA4** — wasatchwise.com, askbeforeyouapp.com, adultaiacademy.com. Add measurement IDs to Next.js, deploy, verify DebugView | ✅ |
| 30 min | **Enable Cloudflare Web Analytics** — wasatchwise.com + askbeforeyouapp.com | ✅ (wasatchwise.com; ABYA not in same CF account) |
| 30 min | **Set up Content Calendar** — Google Sheet, 52 weeks, columns: Week, Date, Title, Status | ✅ (markdown: `EXECUTION_SCHEDULE_2026.md`) |

### WEDNESDAY FEB 11
| Time | Task | Done? |
|------|------|-------|
| 4 hrs MAX | **Pipeline IQ decision** — Can it deliver K-12 leads? YES → fix. NO/UNCERTAIN → sunset. By end of day: either working OR archived + redirect live | ✅ Maintenance mode (Feb 10) |
| 1 hr | **AI Co-Founder prompt** — Use template from Section II.B, generate first 3 blog outlines, save to Content Calendar | ✅ 3 outlines in `WEEK1_BLOG_OUTLINES.md` |
| — | **Blog Post #1 drafted + published** — "5 Questions Superintendents Must Ask Before AI" (~1,200 words) | ✅ Published (`28ffa4e`) |
| — | **Blog Post #2 drafted** — "FERPA vs. COPPA" (~1,200 words) | ✅ Ready to publish |
| — | **MDX blog system + RSS feed** — Full blog infrastructure | ✅ (`f10f10c`) |
| — | **Sabrina Matrix first-visit modals** — One per brand entry point | ✅ (`c625bcd`) |
| — | **Brand nav** — AAA logo by route, ABYA in Brands dropdown, flicker fix | ✅ (`96bea76`, `4f495a4`) |
| — | **AAA page** — Slate palette, in-person seminar, Facebook event | ✅ (`51bf9a0`) |
| — | **WiseBot** — Claude AI integration + audit analysis | ✅ (`66c14b1`) |
| — | **n8n RSS-to-social workflow** | ✅ (`9de774b`) |
| — | **Rock Salt fixes** — Event duplication, ingest-events bugs | ✅ (4 commits) |
| — | **johnlyman.net audit** — Links good, 150+ districts verified | ✅ (remaining: add WasatchWise CTA on homepage) |
| — | **GA4 debug flag removed** | ✅ |

### THURSDAY FEB 12
| Time | Task | Done? |
|------|------|-------|
| — | **Publish Blog Post #2** — "FERPA vs. COPPA" (ready to publish) | ☐ |
| 2 hrs | **johnlyman.net** — Add "Consulting with WasatchWise →" CTA on homepage | ☐ |
| 3 hrs | **SendGrid migration start** — Export contacts (CSV), export templates to Google Drive, create "Email Subscribers Master List" sheet | ☐ |

### FRIDAY FEB 13
| Time | Task | Done? |
|------|------|-------|
| 1 hr | **SendGrid / Resend decision** — Confirm single sender; document decision | ☐ |
| 30 min | **Week 1 checkpoint** — All P0 items complete? Review GA4 data. | ☐ |

### WEEKEND FEB 14-15 (if needed)
- Catch up any incomplete P0 items
- Prep Monday Corporate Briefing template ✅ (already created: `MONDAY_CORPORATE_BRIEFING_TEMPLATE.md`)

---

## WEEK 2: FIRST SHIPS
### MONDAY FEB 16
- **Monday Corporate Briefing** (2 hrs)
- AI Co-Founder generates 3 outlines for Week 3-4
- Select 2 for this week

### TUESDAY FEB 17
- **Publish blog post #1**
- Draft blog post #2

### WEDNESDAY FEB 18
- Build Google Apps Script welcome email sequence
- Test with personal email

### THURSDAY FEB 19
- **Publish blog post #2**
- Complete SendGrid → Google Workspace migration
- Close GitHub security PRs (prioritize wasatchwise, askbeforeyouapp)

### FRIDAY FEB 20
- Week 2 review
- Migrate email subscribers to Google Workspace
- Sunset SendGrid

---

## WEEK 3: CONTENT MACHINE RUNNING
### MONDAY FEB 23
- Monday Corporate Briefing
- Generate outlines for Weeks 5-6

### TUESDAY FEB 24
- Publish blog post #3
- Draft post #4

### WEDNESDAY FEB 25
- Implement n8n workflow: blog → LinkedIn → Twitter

### THURSDAY FEB 26
- Publish blog post #4

### FRIDAY FEB 27
- Set up lead magnets on 2 posts (Cognitive Audit Checklist, AI Policy Template)
- Review Week 1-2 GA4 data (directional only)

---

## WEEK 4: PHASE 1 DONE
### MONDAY MAR 2
- Monday Corporate Briefing

### TUESDAY-WEDNESDAY
- Publish posts #5 and #6

### THURSDAY
- Execute Pipeline IQ decision (if not done: deploy fix OR complete archive)

### FRIDAY MAR 6
- **Phase 1 Retrospective**
- Review 30 days GA4 data
- Hire/contract content editor if needed ($500-1K/mo)
- Generate Month 2 content calendar (Weeks 5-8)

---

## PHASE 2 (WEEKS 5-12): CONTENT SYSTEM BUILD
**Target:** 16 more posts (2/week), 250+ email subs, 1,500+ monthly visitors

### Weekly Rhythm (Lock This In)
| Day | Focus | Hours |
|-----|-------|-------|
| Monday | Corporate Briefing, AI outlines, "What we're NOT doing" | 2 |
| Tuesday | Draft + publish post #1 | 4 |
| Wednesday | Client delivery (Cognitive Audits, consultations) | 6 |
| Thursday | Draft + publish post #2 | 4 |
| Friday | Ops: email sequences, n8n, GitHub, finance | 3 |

### Content Mix
- 8 posts: K-12 AI governance
- 5 posts: Student data privacy + SDPC
- 3 posts: AI literacy (Adult AI Academy preview)

---

## PHASE 3 (WEEKS 9-12): EMAIL AUTOMATION
- Welcome sequence (5 emails)
- Weekly newsletter
- Abandoned lead magnet sequence (3 emails)
- **Target:** 3 sequences live, 25% open rate, 5+ consultation bookings from email

---

## PHASE 4 (MONTHS 4-6): ADULT AI ACADEMY
- Weeks 27-30: Curriculum, site build, 12 video lessons
- Weeks 31-35: Soft launch, paid cert, district pilots
- Weeks 36-39: 4 success posts, referral program
- **Target:** 500+ enrollments, 50+ paid certs, 2 district site licenses

---

## PHASE 5 (MONTHS 7-12): SCALE
- Optimize top 10 posts
- 20 new district contracts
- 30 Utah districts (73% penetration)
- **Target:** $50K MRR, 10K visitors, 2K subscribers

---

## DAILY STANDUP (Solo Version)
Every day, write 3 lines:
1. **Shipped today:** ___________
2. **Shipping tomorrow:** ___________
3. **Blocker:** ___________

---

## THE RULE
If it's on the schedule, you do it. No "I'll do it when I feel like it." No "just one more planning session."

**Week 1 starts now. Execute.**
